...
const AddCopyright = () => {
  ...
  const validate = () => {
    ...
    if (!formData.title.trim()) newErrors.title = "Copyright title is required.";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${BACKEND_API}/copyright/add`, submissionData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || "Failed to submit copyright details.");
      }
    } catch (error) {
      console.error("Error submitting form:", error);
      toast.error("Failed to submit copyright details. Please try again.");
    }
  };
  ...
  return (
    ...
    >
      <div className="conference-form-container">
        <h2>Copyright Submission Form</h2>
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          {/* Copyright Title */}
          <div className="form-group">
            <label htmlFor="title">
              Copyright Title<span className="required">*</span>
            </label>
            <input
              type="text"
              id="title"
